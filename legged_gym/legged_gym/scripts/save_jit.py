...
from rsl_rl.modules.depth_backbone import DepthOnlyFCBackbone58x87, RecurrentDepthBackbone, RecurrentMultiDepthDisconnectResistantBackbone
import argparse
import code
import shutil
...
class HardwareVisionNN(nn.Module):
    def __init__(self,  num_prop,
        ...
        self.depth_encoder = RecurrentMultiDepthDisconnectResistantBackbone(DepthOnlyFCBackbone58x87(num_prop, scan_encoder_dims[-1], depth_encoder_hidden_dim), DepthOnlyFCBackbone58x87(num_prop, scan_encoder_dims[-1], depth_encoder_hidden_dim), num_prop)
        
    def forward(self, obs, depth_img, third_person_depth_img, third_person_disconnected):
        obs_prop_depth = obs[:, :self.num_prop]
        obs_prop_depth[:, 5:7] = 0
        depth_latent_and_yaw = self.depth_encoder(depth_img, third_person_depth_img, third_person_disconnected, obs_prop_depth)
        obs[:, 5:7] = 1.5*depth_latent_and_yaw[:, -2:]
        obs[:, self.num_prop+self.num_scan : self.num_prop+self.num_scan+self.num_priv_explicit] = self.estimator(obs[:, :self.num_prop])
        return self.actor(obs, hist_encoding=True, eval=False, scandots_latent=depth_latent_and_yaw[:, :-2])
...
def play(args):    
    ...
    if "depth_actor_state_dict" in ac_state_dict.keys(): # We are loading a student model
        ...
        policy.depth_encoder.load_state_dict(ac_state_dict['depth_encoder_state_dict'])
        ...
        with torch.no_grad(): 
            ...
            depth_img = torch.ones((num_envs, depth_resized[0], depth_resized[1]), device=device)
            third_person_depth_img = torch.ones((num_envs, depth_resized[0], depth_resized[1]), device=device)
            third_person_disconnected = torch.ones(num_envs, 1, device=device)
            # depth_latent = torch.ones(1, 32, device=device)
            test = policy(obs_input, depth_img, third_person_depth_img, third_person_disconnected)
            
            traced_policy = torch.jit.trace(policy, (obs_input, depth_img, third_person_depth_img, third_person_disconnected))
